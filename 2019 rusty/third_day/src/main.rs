...
fn find_data(x: &mut HashMap<(i32, i32), i32>, s : Vec<&str>, is_find_repeats: bool) -> Vec<((i32, i32),i32)> {
    let mut road : i32 = 0;
    let mut pointer_position = (0i32, 0i32);
    let mut inherits : Vec<((i32, i32),i32)> =  Vec::new();
    for mov in s{
        let repeats : i32 = mov[1 .. ].parse::<i32>().unwrap(); 
        
        match &mov[0 .. 1]{
            "R" => {
                for data in 0 .. repeats {
                    road += 1;
                    pointer_position.0 += 1;
                    push_element(is_find_repeats,x, &mut inherits, pointer_position, road)
                }
            },
            "L" => {
                for data in 0 .. repeats {
...
fn push_element(is_find_repeats: bool,
                x: &mut HashMap<(i32,i32),i32>, 
                inherits: &mut Vec<((i32, i32),i32)>,
                pointer_position: (i32, i32),
                road: i32
            ){
    if is_find_repeats 
    {
        if let Some(road1) = x.get(&pointer_position) 
        {
            inherits.push((pointer_position, road + *road1));                        
        }
    } else  {
        x.insert(pointer_position, road);
    }
}
